...
$(function(){
  ...
  window.Todo = Backbone.Model.extend({
    ...
    EMPTY: "empty todo...",
    urlRoot:'http://localhost:5000/todos',
    // Ensure that each todo created has `content`.
    initialize: function() {
      if (!this.get("content")) {
        this.set({"content": this.EMPTY});
      }else this.set({"content": this.get("content")});
    },
  ...
  window.TodoList = Backbone.Collection.extend({
    ...
    nextOrder: function() {
      if (!this.length) return 1.0;
      return this.last().get('order') + 1.0;
    },
 ...
 //     $(this.el).toggleClass('done', this.model.get('done'))
  ...
  window.AppView = Backbone.View.extend({
    ...
    initialize: function() {
      ...
      Todos.bind('reset', this.render);
      Todos.bind('render', this.addAll);
      var handlers = {
          "success": this.addAll,
